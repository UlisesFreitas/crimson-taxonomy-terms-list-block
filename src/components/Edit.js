	...
	return (
		<div {...blockProps}>
			...
			{isLoading && term && postTypeHasTaxonomy && (
				<Placeholder
					icon="tag"
					label={__('Loading terms…', 'crimson-taxonomy-terms-list-block')}
				/>
			)}
			{term && !postTypeHasTaxonomy && !isLoading && (
				<Placeholder
					icon="tag"
					label={__('Taxonomy Terms List', 'crimson-taxonomy-terms-list-block')}
			...
			{term && !isLoading && postTypeHasTaxonomy && (
				<ServerSideRender
					attributes={attributes}
					block="crimson-taxonomy-terms-list/block"
					urlQueryArgs={{
